...
void fiber_function(fcontext_transfer_t t) {
    ...
    job_manager_t *jm = fiber_data->jm;
...
job_manager_t* create_job_manager(uint32_t num_worker_threads, uint32_t num_fibers, uint32_t fiber_stack_size) {
    job_manager_t *jm = (job_manager_t*)malloc(sizeof(job_manager_t));
...
void stop_job_manager(job_manager_t *jm) {
...
void destroy_job_manager(job_manager_t **jm) {
